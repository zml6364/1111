...
import static com.example.post.MulVquery.MulVquery.i;
import static com.example.post.MulVquery.MulVquery.weldinginfoList;
import static com.example.post.MulVquery.MulVquery.userinfoList;
 ...
 * 用户管理界面业务逻辑
 */
public class MulUserManagerActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void loadUserDb(){
        new Thread(new Runnable() {
            ...
            public void run() {
                userinfoList = null;
                weldinginfoList = queryUtil.parseJson(Mresponse);
    ...
    public void onClick(View v) {
        if(v.getId() == R.id.btn_back)
            ...
            {
                CommonUtils.showDlgMsg(MulUserManagerActivity.this, "最后一页");
            } else { next();}
        }
    }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                mainHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        Intent intent = new Intent(MulUserManagerActivity.this, MulUserManagerActivity2.class);
